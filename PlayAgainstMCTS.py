...
def convert2array(str_board):
	...
	for char in str_board:
		if char == 'z':
			...
			print(board_for_us[int(i/COLUMN_COUNT)][int(i%ROW_COUNT)], ":::")
		i += 1
	print(board_for_us, "|||")
	return board_for_us
...
class MonteCarlo():
	...
	def run_simulation(self):
		...
		for t in range(1, self.max_moves+1):
			...
			moves_states = [(p, to_tuple(next_state_possible(state, p))) for p in legal]
			for p in legal:
				print(pd.DataFrame(next_state_possible(state, p)), p, "move, next_state_possible")
			...
			print(pd.DataFrame(state), "stateeeee")
			self.states.append(state)
			temp = to_tuple(state)
